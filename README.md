...
- `CalcLateRating`: класс для вычисления студентов с задолженностями
...
2. Пользователь запускает программу и передает путь к файлу данных в качестве аргумента командной строки:
   ...
   ```bash
   python -m src.main -p .\data\data.json
   ```

3. Для тестирования системы в качестве аргумента командной строки передается путь к файлу данных:

   ```bash
   pytest test
   ```

4. Программа считывает данные из файла и создает объект класса `CalcLateRating` и `CalcRating`.
5. Объект `CalcLateRating` вычисляет рейтинги студентов и определяет студентов с академической задолженностью. А объект `CalcRating` вычисляет рейтинги студентов.
6. Программа выводит результаты расчетов на экран.
